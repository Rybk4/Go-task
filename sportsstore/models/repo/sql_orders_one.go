...
func (repo *SqlRepository) GetOrder(id int) (order models.Order) {
    ...
    if row.Err() == nil {
        err := row.Scan(&order.ID, &order.ShippingDetails.Name, &order.ShippingDetails.StreetAddr, &order.ShippingDetails.City, &order.ShippingDetails.Zip, &order.ShippingDetails.Country, &order.Shipped)
        if err != nil {
