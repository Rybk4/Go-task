...
func (repo *SqlRepository) SaveProduct(p *models.Product) {
    if p.ID == 0 {
        result, err := repo.Commands.SaveProduct.ExecContext(
            repo.Context,
            p.Name, p.Description, p.Category.ID, p.Price,
        )
        if err == nil {
        ...
        result, err := repo.Commands.UpdateProduct.ExecContext(
            repo.Context,
            p.Name, p.Description, p.Category.ID, p.Price, p.ID,
        )
        if err == nil {
